...
namespace Spark::SandBox
{
    ...
    void DrawCube::CreatePasses()
    {
        ...
        // ScenePass: render the cube into 4x MSAA transient targets, resolved into the swap
        // chain when the render pass ends.
        SPARK_RENDER_PASS(passContext, "ScenePass")
            ...
            .Build([this](Spark::Render::RenderPassScopes& p)
            {
                ...
                auto s = p.Scope();
                auto color = s.RenderTarget(RHI::AttachmentId("MSAAColor"), colorAction);
