...
```
预算总额         kShadowBudgetUnits = 64
level 1 (2048²)  ShadowTileCost = 16
6 面              6 × 16 = 96 > 64
```

**一盏点光在最粗档连整张图集都装不下。** 但这不需要新策略——只要预算阶段把成本算成 `面数 × ShadowTileCost(level)`，§七 已有的降级逻辑自动把它压到 level 2（6×4 = 24）。

所以面剔除真正的收益**不是省 tile 数量，是让点光够得着更高的分辨率**：N 降到 2~4 之后，同样的降级逻辑会给出更粗的档。

#### 剔除的判据

每个面是从光源出发的 90° 视锥，判据是它与主视锥是否相交。

**不能用包围球。** 90° 棱锥的最小外接球半径是 `1.414 × range`（角上半角 54.7°），**比光源自己的包围球还大**。六个这样的球互相重叠，只有整盏灯出画时才会拒绝，等于没剔。

用**保守拒绝**：面视锥有 5 个顶点（光源 + 4 个远端角）。逐个检查主视锥的 6 个平面，若 5 个顶点全在某一平面外侧则该面必定不可见。

- 30 次点积/面，180 次/灯
- 有假阳性（多渲染几个面），**没有假阴性**——与 6a 的「漏而非错杀」同向
- `Math::Frustum` 加一个凸包保守拒绝的方法即可，平面数据现成
- `ViewFrustum` 组件在 §六 第 2 步就为此埋了伏笔

省得最多的是**远处的灯**（从远光源看，可见区域只张开一个小角）；近处的灯省得少，因为可见区域几乎铺满所有方向。方向与成本相反，但远灯数量多，总量上仍显著。

#### 行永远 6 个，tile 只给存活的面

面数可变会打破 `base + faceIndex` 寻址：只有面 0/2/5 存活时，行 base+0/1/2 装的是面 0/2/5，shader 算出面号 5 去查 base+5 就错位。

**两者的稀缺程度差着数量级**——一行是 96 字节的缓冲区条目，一块 tile 是 512²~2048² 的图集面积。拿便宜的换寻址的简单性。

被剔除的面在它那一行留无效标记（退化的 `m_uvMinMax`），shader 读 `base + face` 见无效即返回受光——**shader 侧不用加分支**，它本来就要处理「这盏灯没有阴影」。

`kShadowViewCapacity` 随之要往上调（现在 64，按 6 行一盏只够 10 盏点光）。

#### 其余清单

已经就绪：tile 编号与行号分离（面可散落图集任意位置，行号仍连续）、超预算降级、`Deactivate`/sweep 已按 `m_views` 循环。

还要做：

- 行号的**连续段**分配（现在 `AllocateViewIndex` 只发单个）
- tile 的**原子**分配：存活的 N 个面要么全给要么不给，半个 cube 比没有阴影更难看
- 一盏灯的面**等级统一**，降级整体降，否则面之间滤波质量不一致
- 预算把一盏点光当**一个不可分的候选**，成本 `N × ShadowTileCost`
...
**未决的策略**：一盏点光要 N 块但预算只剩不到 N 块时，是就此打住还是跳过它让后面要 1 块的灯用掉？跳过会让名次不再单调（与 §七 拒绝「跳过」的理由同源），但打住会浪费。§七 的降级在这里只解决一半——降级改变尺寸，而**面数不能降**。面剔除让 N 常见是 2~4 而非恒 6，撞上这个边界的概率大幅降低，但问题仍在。
