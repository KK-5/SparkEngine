...
float4 VSMain(float3 pos : POSITION) : SV_Position
{
    ...
    float4 v4    = g_TemporalAAJitter + g_ViewRectMin + g_InputViewRectMin + g_ViewSizeAndInvSize + g_BufferSizeAndInvSize + g_InputBufferSizeAndInvSize + g_InvDeviceZToViewZ;
    float  s     = g_Exposure + g_PreExposure + g_OneOverPreExposure + (float)g_FrameNumber + g_GameTime + g_PrevGameTime + g_DeltaTime;
