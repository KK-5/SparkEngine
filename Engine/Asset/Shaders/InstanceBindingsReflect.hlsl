...
//
// InstanceBindings.hlsli declares a StructuredBuffer with no entry point, and the
// asset builder only compiles shaders that define a known entry point — so it
// cannot reflect InstanceBindings.hlsli on its own. This file gives the group a
