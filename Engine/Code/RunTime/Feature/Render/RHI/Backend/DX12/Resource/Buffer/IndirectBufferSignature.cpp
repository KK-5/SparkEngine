...
namespace Spark::RHI::DX12
{
    ...
    RHI::ResultCode IndirectBufferSignature::InitInternal(RHI::Device& deviceBase, const RHI::IndirectBufferSignatureDescriptor& descriptor)
    {
        ...
        eastl::vector<D3D12_INDIRECT_ARGUMENT_DESC> argumentsDesc(commands.size());
        m_stride = 0;
        ...
        for (uint32_t i = 0; i < commands.size(); ++i)
        {
            ...
            case RHI::IndirectCommandType::RootConstants:
            {
                if (!pipelineState)
                {
                    ASSERT(pipelineState, "PipelineState is required when using inline constant commands");
                ...
                if (!pipelineLayout)
                {
                    ASSERT(pipelineState, "PipelineLayout is null");
                    return RHI::ResultCode::InvalidArgument;
                }
            ...
            default:
                ASSERT(false, "Invalid indirect argument type");
                return RHI::ResultCode::InvalidArgument;
            }
        }
