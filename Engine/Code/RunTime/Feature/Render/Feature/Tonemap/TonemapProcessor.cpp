...
namespace Spark::Render
{
    void TonemapProcessor::Init()
    {
        ...
        // Persistent draw-request carrier, tagged for this pass. Its DrawItem is produced
        // by CompileDrawRequests each frame like every other pass.
        m_drawRequest = rhiCtx->CreateEntity();
        rhiCtx->Add<SPARK_PASS_TAG("TonemapPass")>(m_drawRequest);
    ...
    void TonemapProcessor::Shutdown()
    {
        auto* rhiCtx = RHI::RHIExecuteContext::Current();
        if (!rhiCtx)
        {
    ...
    void TonemapProcessor::Process(const Math::Vector2Int& renderSize)
    {
        auto* rhiCtx  = RHI::RHIExecuteContext::Current();
